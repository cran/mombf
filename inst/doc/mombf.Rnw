...
We will perform model selection assuming that the residual variance \texttt{phi} is known.
The routine \texttt{modelSelection} implements a Gibbs sampling scheme.
Additionally to the the posterior sample, which is saved in the slot \texttt{postSample},
the routines also stores the model with highest posterior probability amongst all those considered
during the Gibbs scheme and the marginal posterior probabilities of inclusion for each covariate.
