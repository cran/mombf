...
The intuitive appeal of Mom and iMom BF is that they represent prior beliefs under the alternative hypothesis
...
<<label=fig1plot,include=FALSE>>=
 ...
 lines(thseq,dmom(thseq,g=g,baseDensity='t',nu=3),lty=2,col=2)
 lines(thseq,dimom(thseq,g=g),lty=3,col=3)
@
...
<<label=fig1bplot,include=FALSE>>=
 library(mombf)
 plot(thseq,pmom(thseq,g=g),type='l',ylab='Prior cdf')
 lines(thseq,pimom(thseq,g=g),lty=3,col=3)
@
...
@
\end{center}
\caption{Moment and inverse Moment cdf for $g=1$}
\label{fig:cdfplot}
\end{figure}
...
about the standardized regression coefficient $(\theta_1/(\sigma n V_1)^2$.
...
gtmom <- mode2g(prior.mode,prior='tMom',nu=3)
...
lines(thseq,dmom(thseq,V1=V[2,2],g=gtmom,n=nrow(hald),baseDensity='t',nu=3),lty=2,col=2)
lines(thseq,dimom(thseq,V1=V[2,2],g=gimom,n=nrow(hald)),lty=3,col=3)
@
...
<<twobis>>=
a <- .2; priorp <- .05
gmom2 <- priorp2g(priorp=priorp,q=a,prior='normalMom')
gimom2 <- priorp2g(priorp=priorp,q=-a,prior='iMom')
gmom2
gimom2
@
...
When $\sigma^2$ is unknown, \texttt{method=='adapt'} combines
...
of the posterior distribution of $\sigma^2$ at which to evaluate
...
mombf(lm1,coef=2,g=gtmom,baseDensity='t')
imombf(lm1,coef=2,g=gimom,method='adapt')
imombf(lm1,coef=2,g=gimom,method='MC',B=10^5)
