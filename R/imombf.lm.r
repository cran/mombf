imombf.lm <- function(lm1,coef,g,prior.mode,nu=1,theta0,B=10^5) {
#Inverse Moment Bayes factor for linear models. Wrapper to make it easier to call imomunknown
#Input:
...
if (missing(theta0)) theta0 <- rep(0,length(coef)) else if (length(theta0)!=length(coef)) stop('theta0 must have the same length as coef')
  ...
  bf.imom <- imomunknown(thetahat[coef],V[coef,coef],n,nuisance.theta=p-p1,g=g,nu=nu,theta0=theta0,ssr=ssr,B=B)
  return(bf.imom)
}
