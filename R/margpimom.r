...
fpimomNeg <- function(th, XtX, ytX, phi, tau) (XtX %*% matrix(th,ncol=1) - t(ytX))/phi - 2*tau*phi/th^3 + 1/th
fppimomNeg <- function(th, XtX, ytX, phi, tau) XtX/phi + diag(6*tau*phi/th^4 - 1/th^2, ncol=length(th)) 
...
pimomMarginalU <- function(sel, y, x, alpha=0.001, lambda=0.001, tau=1, method='Hybrid', B=10^5, logscale=TRUE, XtX, ytX) {
...
# - tau: prior dispersion parameter
# - method: method to approximate the integral for known phi. Integral wrt phi is performed via integrate. 'Laplace' for Laplace approx which may underestimate true value, 'MC' for exact evaluation which can be very computationally expensive. 'Hybrid' uses Laplace approx for all phi values, and then estimates the Laplace error by doing a single exact evaluation for a value of phi close to the posterior mode
# - B: number of Monte Carlo samples to use (ignored unless method=='MC')
  if (is.matrix(y)) y <- as.vector(y)
...
pimomMarginalUR <- function(y, x, alpha=0.001, lambda=0.001, tau=1, method='Hybrid', B=10^5, logscale=TRUE, XtX, ytX) {
#Marginal density of the data y~N(x*theta,phi*I) under a product imom prior (unknown variance)
# - y: response variable
# - x: design matrix
# - alpha, lambda: prior for phi is IGamma(alpha/2,lambda/2)
...
# - B: number of Monte Carlo samples to use (ignored unless method=='MC')
  ...
  if (method=='Hybrid') {
    ...
    adj <- intmc/intlapl
    ans <- integrate(f2int, 0, Inf, method='Laplace', adj=adj)
  } else {
    ans <- integrate(f2int, 0, Inf, method=method, adj=1)
  }
  if (logscale) ans <- log(ans$value)
  return(ans)
}
